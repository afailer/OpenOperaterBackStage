<template>
    <div class="basisInfo" v-loading="contentLoading">
        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    ...
                    <el-button 
                    v-if="basis.policy_state_can_modify===1&&(user.permissionPath&&(user.permissionPath.includes('/v2/policy/reinstate')||user.permissionPath.includes('*')))"
                    type="primary" 
                    size="mini" 
                    @click="recover"
                    ...
                    (user.permissionPath&&
                    (user.permissionPath.includes('/v2/policy/ineffective')
            ...
            <el-table-column align="center" prop="is_entrust" label="自动续费" width="120">
                <template slot-scope="scope">
                    <el-switch
            ...
            <el-table-column align="center" prop="auto_renew" label="自动续保" width="120">
                <template slot-scope="scope">
                    <el-switch
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    <template v-for="item in scope.row.badges">
                        <el-tag :key="item" style="margin-left: 10px">{{item}}</el-tag>
                    </template>
                </template>
            </el-table-column>
            <el-table-column
        ...
        width="400px">
            ...
            <el-row style="margin-top:20px">宽限期：{{expire_day}}天</el-row>
...
export default {
    ...
    mounted() {
        ...
        this.$store.dispatch('GetPolicyDetailNew', policyId)
    },
    computed: {
        ...mapGetters([
            'accountListNew',
        ...
        basis() {
            return this.policyDetailNew.basis
        },
        basisArray() {
            if(this.policyDetailNew.basisArray[0].is_entrust==='是'){
                this.switch_is_entrust=true
            }else{
                this.switch_is_entrust=false
            }
            if(this.policyDetailNew.basisArray[0].auto_renew==='是'){
                this.switch_auto_renew=true
            }else{
                this.switch_auto_renew=false
            }
            return this.policyDetailNew.basisArray
        }
    },
    methods: {
        //返回上一级
        onReturn() {
            ...
            if(redirect) {
                ...
                this.$router.push('/qurey-insur-policy-new')
            }
        },
        changeEntrustFunc(newVal){
            ...
            commonApi.changeEntrust(
                vm.policyDetailNew.basisArray[0].policy_id,
                'period',
                value
            ).then(res => {
        ...
        recover(){
            this.expire_day=this.policyDetailNew.basis.days_grace
        ...
        activeReturn(row){
            ...
            vm.$confirm('确认退保吗?', '提示', {
                ...
                commonApi.activeReturnNew(
                    ...
                    row.state='保单失效'
                    vm.policyDetailNew.basis.policy_state_can_modify=1
                    vm.$forceUpdate()
                })
                .catch(function(err) {
                    vm.$loading().close()
                ...
                if(vm.state===20){
                    ...
                    commonApi.modifyStateNew(
                        ...
                        if(res.code===0){
                            ...
                            vm.policyDetailNew.basisArray[0].state="投保成功"
                            vm.policyDetailNew.basis.policy_state_can_modify=0
